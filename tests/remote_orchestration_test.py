...
from sollertia_forgery.managing import PROJECT_JOBS_SCHEMA
...
def isolated_working_directory(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    The ledger is written under that directory, so every test in this module records into its own file rather than
    into whatever this host already holds.
    """
    data_directory = tmp_path.joinpath("user_data")
    data_directory.mkdir()
    monkeypatch.setattr(platformdirs, "user_data_dir", lambda appname, appauthor: str(data_directory))  # noqa: ARG005
...
def test_a_job_whose_upstream_stage_cannot_run_is_blocked_rather_than_submitted() -> None:
    ...
    jobs = [
        descriptor(job_id="rename", job_name="rename", prerequisite_ids=("timestamp_absent",)),
...
def test_blocking_propagates_down_the_chain() -> None:
    """A stage waiting on a blocked stage is blocked in turn rather than being submitted behind it."""
    jobs = [
        descriptor(job_id="root", job_name="binarize", prerequisite_ids=("missing",)),
        descriptor(job_id="middle", job_name="process", prerequisite_ids=("root",)),
...
def test_a_prerequisite_the_batch_holds_does_not_block_its_dependent() -> None:
    ...
    jobs = [
        descriptor(job_id="timestamp", job_name="timestamp", specifier="1"),
...
def test_a_submission_names_the_allocations_of_the_upstream_jobs_the_batch_holds() -> None:
    ...
    submissions = submit_batch(server=server, jobs=jobs, batch_id="batch01")  # type: ignore[arg-type]
...
def test_a_submission_requests_the_cores_and_memory_the_job_was_prepared_at() -> None:
    ...
    server = StubServer()
    jobs = [descriptor(job_id="energy", job_name="motion_energy", specifier="1", cores=16, memory_mb=5000)]

    submit_batch(server=server, jobs=jobs, batch_id="batch01")  # type: ignore[arg-type]
    ...
    assert f"#SBATCH --time={REMOTE_JOB_WALLTIME_MINUTES // 60:02d}:00:00" in script
...
def test_every_allocation_writes_into_the_batch_directory() -> None:
    ...
    server = StubServer()
    jobs = [descriptor(job_id="energy", job_name="motion_energy", specifier="1")]
...
def test_the_script_exits_with_the_status_of_the_work_it_ran(tmp_path: Path) -> None:
    ...
    A cleanup command placed last would make every job report success, which would let a dependent allocation run
    against input its upstream stage never produced.
    """
    job = Job(
        job_name="job",
...
def test_rendering_a_script_twice_produces_the_same_script(tmp_path: Path) -> None:
    ...
    assert job.command_script == job.command_script
...
def submission(slurm_job_id: str, job_id: str = "job") -> RemoteSubmission:
...
def batch(batch_id: str, submissions: list[RemoteSubmission], submitted_at: int = 1) -> SubmissionBatch:
...
def test_the_ledger_round_trips_through_yaml() -> None:
    ...
    record_batch(batch=batch(batch_id="batch01", submissions=[submission(slurm_job_id="1000")]))

    recorded = read_ledger().batch(batch_id="batch01")

    assert ledger_path().is_file()
    assert recorded is not None
    assert recorded.walltime_minutes == REMOTE_JOB_WALLTIME_MINUTES
    assert recorded.submissions[0] == submission(slurm_job_id="1000")
...
def test_allocations_accepted_before_a_rejection_are_still_recorded() -> None:
    ...
    jobs = [descriptor(job_id=f"job{index}", job_name="motion_energy", specifier=str(index)) for index in range(4)]

    with pytest.raises(RuntimeError):
        submit_batch(server=RejectingServer(), jobs=jobs, batch_id="batch01")  # type: ignore[arg-type]
...
def test_recording_a_batch_preserves_the_batches_already_in_the_ledger() -> None:
    ...
    record_batch(batch=batch(batch_id="batch01", submissions=[submission(slurm_job_id="1000")]))
    record_batch(batch=batch(batch_id="batch02", submissions=[submission(slurm_job_id="2000")]))
...
def test_re_recording_a_batch_replaces_its_earlier_record() -> None:
    ...
    record_batch(batch=batch(batch_id="batch01", submissions=[submission(slurm_job_id="1000")]))
    record_batch(
        batch=batch(batch_id="batch01", submissions=[submission(slurm_job_id="1000"), submission(slurm_job_id="1001")])
    )

    ledger = read_ledger()
    assert len(ledger.batches) == 1
    assert len(ledger.batches[0].submissions) == 2
...
def test_a_partly_finished_batch_is_retained() -> None:
    ...
    assert retired == []
    assert read_ledger().batch(batch_id="batch01") is not None
...
def test_retirement_leaves_the_batches_that_are_still_running() -> None:
    ...
    record_batch(batch=batch(batch_id="done", submissions=[submission(slurm_job_id="1")]))
    record_batch(batch=batch(batch_id="running", submissions=[submission(slurm_job_id="2")]))
...
def test_naming_no_batch_resolves_every_outstanding_batch() -> None:
    ...
    record_batch(batch=batch(batch_id="batch01", submissions=[submission(slurm_job_id="1")]))
    record_batch(batch=batch(batch_id="batch02", submissions=[submission(slurm_job_id="2")]))
...
def plan_frame(rows: list[dict[str, Any]]) -> pl.DataFrame:
...
def state_frame(rows: list[dict[str, Any]]) -> pl.DataFrame:
