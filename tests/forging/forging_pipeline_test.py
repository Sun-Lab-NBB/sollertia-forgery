...
"""The name under which every forged dataset in this module is defined, carrying uppercase so the lowercasing of the
cindra output directory stays observable."""

_MULTIDAY_CONFIGURATION_FILENAME: str = "multi_recording_configuration.yaml"
"""The filename under which each tracked animal's materialized multi-recording configuration is written."""

_ARGUMENT_RECORD_FILENAME: str = "assembly_arguments.txt"
"""The filename into which the stand-in assembly worker records its source path and dataset name, which lets a job
dispatched into a worker process report its own arguments."""
...
"""A column name that no acquisition system describes, which the assembly stage rejects."""

_GENOTYPE: str = "GP5.17"
"""The genotype recorded by every synthetic animal, which the Mesoscope-VR resolver maps to a calcium indicator."""

_PRIMING_STEP: str = "priming"
"""The label with which the shared dispatch recorder marks a bootstrap priming call, which collides with no cindra job
name."""


def _assemble_described_session(source_session_path: Path, output_path: Path, dataset_name: str) -> None:
...
def _assemble_undescribed_session(source_session_path: Path, output_path: Path, dataset_name: str) -> None:
...
def _assemble_failing_session(source_session_path: Path, output_path: Path, dataset_name: str) -> None:
...
class _MultidayCall:
    ...
    Attributes:
        configuration_path: The configuration the invocation was handed.
        job_name: The cindra job the invocation requested.
        specifier: The recording identifier the invocation named.
        job_id: The forging tracker identifier against which the invocation recorded.
...
def _write_surgery_metadata(session: SessionData) -> None:
    """Writes the surgery metadata snapshot from which the cindra configuration resolver reads the animal's genotype.
...
def recorded_dispatch_order() -> list[tuple[str, str]]:
    """Returns the one timeline to which both cross-recording recorders append, in the order the pipeline reached them.

    Priming and dispatch are separate calls, so recording them into two lists makes only their contents assertable.
    A single interleaved timeline makes the order between them observable, which is the guarantee that
    ...
    Returns:
        The list to which every priming and every dispatched stage is appended, as a ``(step, subject)`` pair naming
        the animal that owns a priming or a whole-animal stage and the session a per-recording stage names.
    """
    return []
...
def recorded_primings(monkeypatch: pytest.MonkeyPatch, recorded_dispatch_order: list[tuple[str, str]]) -> list[Path]:
    ...
    Args:
        ...
        recorded_dispatch_order: The shared timeline to which each priming is appended alongside the dispatched stages.

    Returns:
        The list to which every primed configuration path is appended, in call order.
    """
    primed: list[Path] = []
...
def recorded_multiday_jobs(
    ...
    Args:
        ...
        recorded_primings: The recorder standing in for the dataset priming that the discovery stage performs first.
        recorded_dispatch_order: The shared timeline to which each dispatch is appended alongside the primings.

    Returns:
        The list to which every dispatched invocation is appended, in dispatch order.
    """
    calls: list[_MultidayCall] = []
    ...
    def _execute(
        ...
        *,
        ...
        # A stage spanning the whole animal carries no specifier, so it is named by the animal that owns its
        # configuration, which is the same subject under which the priming of that animal is recorded.
...
def install_assembly_worker(monkeypatch: pytest.MonkeyPatch) -> Callable[[Any], None]:
    ...
    Returns:
        A callable taking the worker to which every acquisition system resolves.
    """
    ...
    def _install(worker: Any) -> None:
        monkeypatch.setattr(
            ...
            lambda system: worker,  # noqa: ARG005 - the stand-in resolves one worker for every system.
        )
...
def experiment_project(
    ...
    Every session is fully processed and carries its surgery metadata, which is the state that forging admission and
    the cross-recording configuration resolver both require.

    Args:
        project_root: The created project under which the sessions are placed.
    ...
    for session in sessions:
        ...
        _write_surgery_metadata(session=session)
    return _ForgingProject(project_root=project_root, sessions=tuple(sessions))
...
def _session_entry(dataset: DatasetData, name: str) -> Any:
...
def test_define_forging_dataset_names_a_repeated_session_once(experiment_project: _ForgingProject) -> None:
...
def test_define_forging_dataset_skips_an_animal_without_cross_recording_tracking(
    training_project: _ForgingProject,
) -> None:
...
def test_define_forging_dataset_records_the_progress_preference(experiment_project: _ForgingProject) -> None:
...
def test_define_forging_dataset_resets_a_rebuilt_animals_recorded_jobs(
    experiment_project: _ForgingProject,
    recorded_multiday_jobs: list[_MultidayCall],  # Requested so the cross-recording stages succeed.
    ...
    """
    install_assembly_worker(_assemble_described_session)
    names = experiment_project.names()
    ...
    states = _tracker_states(dataset=rebuilt)
...
def test_load_multiday_plan_skips_an_animal_holding_no_sessions(tmp_path: Path) -> None:
    """Verifies that an animal listed by the dataset without any session contributes no plan entry."""
    ...
    dataset = SimpleNamespace(
        ...
        get_sessions_for_animal=lambda animal: (),  # noqa: ARG005 - the stub holds no session.
    )
...
def test_define_forging_dataset_rewrites_a_configuration_that_was_never_written(
    ...
    # The dataset marker is committed before the configurations are written, so a definition that died in between
    # leaves an animal that the marker names and the disk does not.
    dataset.get_animal(animal="321").animal_path.joinpath(_MULTIDAY_CONFIGURATION_FILENAME).unlink()
    ...
    # Reading the plan back passes such an animal over in silence, so its discovery and extraction stages vanish from
    # the job universe until a repeated definition repairs it.
...
def test_define_forging_dataset_passes_over_an_unconfigured_animal_whose_sources_moved_away(
    ...
    """Verifies that an animal that the dataset holds without a configuration is left as it stands once its source data
    has moved off this machine, so a large project is still forged in passes.
    """
    names = experiment_project.names()
    dataset = define_forging_dataset(
    ...
    # Materializing a configuration loads every one of the animal's sessions from the project root, so an animal
    # whose directory is gone cannot be repaired and attempting it would fail the whole definition.
...
def test_define_forging_dataset_extends_a_dataset_whose_forged_animal_lost_its_sources(
    ...
    """Verifies that adding an animal succeeds while an animal already in the dataset has no source data left."""
    names = experiment_project.names()
...
def test_discover_forging_jobs_reads_no_source_data(experiment_project: _ForgingProject) -> None:
...
def test_discover_forging_jobs_reports_the_whole_universe_as_possible(experiment_project: _ForgingProject) -> None:
...
def _define_whole_project(experiment_project: _ForgingProject) -> DatasetData:
    """Defines the module's dataset over every session the project holds.

    Args:
        experiment_project: The source project from whose sessions the dataset is defined.
...
def test_run_forging_pipeline_completes_every_stage_across_a_worker_pool(
    ...
    its extractions before moving to the next animal. The run records every stage as succeeded and re-exports each
    ...
    # The cross-recording steps are compared as one ordered timeline rather than counted per kind, because the order
    # is the guarantee. An extraction reads the ROI set its animal's discovery wrote, and every stage reads the
    # bootstrap the priming wrote ahead of that animal's first stage.
    ...
    states = _tracker_states(dataset=DatasetData.load(dataset_path=dataset.dataset_data_path.parent))
    ...
    for source in experiment_project.sessions:
        entry = _session_entry(dataset=dataset, name=source.session_name)
...
def test_run_forging_pipeline_assembles_sequentially_with_one_worker(
    ...
    first = _session_entry(dataset=dataset, name=names[0])
    ...
    assert set(_tracker_states(dataset=dataset).values()) == {ProcessingStatus.SUCCEEDED}
...
def test_run_forging_pipeline_assembles_at_the_default_worker_count(
    ...
    # The documented default asks for every available core by spelling the request as a negative number, so the
    # request itself is never a pool width. The host's resolved core count is pinned here so the parallel branch is
    # the one taken wherever this suite runs.
    monkeypatch.setattr(
        pipeline_module,
        "resolve_worker_count",
        lambda requested_workers: 16,  # noqa: ARG005 - the stand-in pins the count whatever is requested.
    )
    install_assembly_worker(_assemble_described_session)
    dataset = define_forging_dataset(
        name=_DATASET_NAME, session_names=experiment_project.names(), project_root=experiment_project.project_root
...
def test_run_forging_pipeline_registers_the_only_job_of_a_single_session_dataset(
    ...
    assert _session_entry(dataset=dataset, name=names[0]).data_path.is_file()
    # A training session carries no cross-recording stages, so a dataset holding one of them has a universe of exactly
    # one assembly job. A tracker refuses a job it was never told to track, so a run that skipped the registration
    # would die on the first session it assembled rather than forge it.
    assert _tracker_states(dataset=dataset) == {(FORGING_JOB_NAME, names[0]): ProcessingStatus.SUCCEEDED}
...
def test_run_forging_pipeline_resolves_the_assembly_worker_under_the_acquisition_system(
    ...
    def _resolve(system: str) -> Any:
        """Records the key under which the pipeline resolved the per-session assembly worker."""
        requested.append(system)
        return _assemble_described_session
    ...
    # The registry is keyed by acquisition system alone, so a lookup spelled in any other vocabulary resolves no
    # worker and the run dies in the registry before a single session is assembled.
...
def test_run_forging_pipeline_reexports_only_the_assets_a_training_session_carries(
    ...
    states = _tracker_states(dataset=dataset)
    assert set(states) == {(FORGING_JOB_NAME, name) for name in names}
    output_directory = _session_entry(dataset=dataset, name=names[0]).data_path.parent
...
def test_run_forging_pipeline_skips_the_stages_already_recorded_as_succeeded(
    experiment_project: _ForgingProject,
    recorded_multiday_jobs: list[_MultidayCall],
    install_assembly_worker: Callable[[Any], None],
) -> None:
    ...
    assert not _session_entry(dataset=dataset, name=names[0]).data_path.exists()
    assert _session_entry(dataset=dataset, name=names[1]).data_path.is_file()
    ...
    assert set(_tracker_states(dataset=dataset)) == set(universe)
    assert set(_tracker_states(dataset=dataset).values()) == {ProcessingStatus.SUCCEEDED}
...
def test_run_forging_pipeline_reports_a_fully_assembled_dataset(
    ...
    install_assembly_worker(_assemble_failing_session)
    run_forging_pipeline(name=_DATASET_NAME, project_root=experiment_project.project_root, workers=1)
...
def test_run_forging_pipeline_rejects_a_session_missing_a_required_asset(
    ...
    assert not _session_entry(dataset=dataset, name=names[0]).data_path.exists()
    assert _tracker_states(dataset=dataset)[FORGING_JOB_NAME, names[0]] == ProcessingStatus.FAILED
...
def test_run_forging_pipeline_rejects_an_undescribed_column(
    ...
    """Verifies that a feather carrying a column that the dataset does not describe fails its own session's assembly."""
    install_assembly_worker(_assemble_undescribed_session)
    names = experiment_project.names()
    dataset = define_forging_dataset(
        name=_DATASET_NAME, session_names=(names[0],), project_root=experiment_project.project_root
    )
...
def test_run_forging_pipeline_rejects_an_unknown_job_id(
    ...
    """Verifies that a job identifier that the dataset does not carry is refused rather than silently skipped."""
    install_assembly_worker(_assemble_described_session)
    define_forging_dataset(
        name=_DATASET_NAME, session_names=experiment_project.names(), project_root=experiment_project.project_root
    )
...
def test_run_forging_pipeline_runs_the_named_discovery_job_alone(
    ...
    assert _tracker_states(dataset=dataset)[MULTIDAY_DISCOVERY_JOB_NAME, "305"] == ProcessingStatus.SUCCEEDED
...
def test_run_forging_pipeline_normalizes_a_non_positive_worker_request(
    ...
    # Every worker count below one means every available core throughout this library, but cindra spells that request
    # as -1 alone and rejects every other non-positive value, so the stage would refuse an un-normalized zero.
