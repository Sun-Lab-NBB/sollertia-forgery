...
def _assemble_2p_fluorescence_dataset(session_data_path: Path, multiday_data_path: Path) -> pl.DataFrame:
    """Assembles the target session's 2-photon fluorescence dataset from the data generated by the cindra's
    ...
    single_day_data = session_data_path.joinpath("processed_data", "mesoscope_data", "cindra", "combined")

    # Resolves the path to the behavior data directory
    behavior_data = session_data_path.joinpath("processed_data", "behavior_data")

    # Queries the number of frames processed by cindra. This is used to handle rare cases where the log has
    # more frame stamps than recorded frames, which usually happens if the user manually triggers mesoscope scanning
    # (of any kind) outside the expected time slot. Specifically, if the log has more frames than the cindra data, the
...
def _rectify_mesoscope_vr_experiment_state_assignment(
    ...
    sollertia-experiment library versions.
...
def _assemble_experiment_dataset(session_data_path: Path, reference_time: NDArray[np.uint64]) -> pl.DataFrame:
    """Assembles the target session's experiment dataset from the experiment metadata generated by the sollertia-forgery processing
    processing pipeline.
...
def _assemble_behavior_dataset(
    ...
    """Assembles the target session's behavior dataset from the data generated by the sollertia-forgery processing processing pipeline.
