...
def assemble_cindra_dataset(cindra_data_path: Path, behavior_data_path: Path, multiday_data_path: Path) -> pl.DataFrame:
    ...
    _, frames = np.load(cindra_data_path.joinpath("cell_fluorescence.npy"), mmap_mode="r").shape
    ...
    with np.load(cindra_data_path.joinpath("combined_metadata.npz")) as metadata:
    ...
    mesoscope_frame_data = pl.read_ipc(
        behavior_data_path.joinpath("mesoscope_frame_data.feather"), memory_map=True, rechunk=True
    )
    ...
    frame_aligned_data: pl.DataFrame = (
        ...
        .with_columns(((pl.col("pulse_end") - pl.col("pulse_start")) / 1000).alias("duration_ms"))
    ...
    classification = np.load(cindra_data_path.joinpath("cell_classification.npy"), mmap_mode="r")
