...
class AssemblyGeometry:
    ...
    Notes:
        ...
        arrive on the clock on which each of them was sampled. A fast camera writes far more samples than the reference
        clock on which the assembly settles, so the two families diverge by the ratio between those rates and neither
        ...
        source costs, are properties of the assembly stage rather than of the data. The system that donates this record
        answers only for its own data.
    """
    ...
    """The samples held by each source the assembly reads, counted on that source's own clock, one entry per source.
    A source the assembly reads through several files of equal height contributes one entry, since those files share a
    clock.
    """
