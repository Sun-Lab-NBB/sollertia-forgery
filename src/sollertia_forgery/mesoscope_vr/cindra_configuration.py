"""Provides the Mesoscope-VR cindra configuration resolvers donated to the system-agnostic two-photon and forging
pipelines.

Each resolver reads the animal's genotype from the session's surgery metadata, selects the calcium indicator, and
builds the lab-tuned CA1 single- or multi-recording configuration in code, so the pipelines materialize a runnable
cindra configuration without an external configuration file. The configuration is written out in full rather than
relying on cindra defaults, so a change to a cindra default cannot silently alter the science. Only the
indicator-dependent parameters (the OASIS decay ``tau``, the neuropil coefficient, and the multi-recording
ROI-selection probability threshold) vary between genotypes, and cross-recording cell tracking applies only to
experiment sessions.
"""
...
class CalciumIndicator(StrEnum):
    ...
    JGCAMP8S = "jGCaMP8s"
    """The AAV-delivered jGCaMP8s indicator, the slow-decay member of the jGCaMP8 family."""
...
collapsed). Only the two lab indicators are recognized, so an unrecognized genotype fails loudly rather than defaulting
to a possibly-wrong sensor."""
...
def _build_single_recording_configuration(genotype: str) -> SingleRecordingConfiguration:
    """Builds the lab-tuned CA1 single-recording cindra configuration for an animal's genotype.
...
def _build_multi_recording_configuration(genotype: str) -> MultiRecordingConfiguration:
    """Builds the lab-tuned CA1 multi-recording cindra configuration for an animal's genotype.
