...
from .dataset import SessionTypes
...
def _assemble_experiment_dataset(session_data_path: Path, reference_time: NDArray[np.uint64]) -> pl.DataFrame:
    ...
    # Starts building the aligned dataframe with the encoder data interpolated (downsampled) to the reference time
    # array.
    ...
    # Extracts the interpolated traveled distance used to align the data sources stored in relation to the traveled
    ...
    return result_df.with_columns(pl.col("trial").cast(pl.UInt16), pl.col("traveled_distance_cm").round(2))
...
def _assemble_behavior_dataset(session_data_path: Path, reference_time: NDArray[np.uint64]) -> pl.DataFrame:
    ...
    if behavior_data_path.joinpath("encoder_data.feather").exists():
        ...
        aligned_data["running_speed_cm_s"] = interpolate_data(
            source_coordinates=encoder_time,
    ...
    if behavior_data_path.joinpath("screen_data.feather").exists():
        ...
        aligned_data["screens_active"] = interpolate_data(
    ...
    if behavior_data_path.joinpath("break_data.feather").exists():
        ...
        aligned_data["break_active"] = (break_torque > 0).astype(np.uint8)
    ...
    behavior_data = (
        ...
        .with_columns(
            ...
            pl.col("dispensed_water_volume_uL").sum().over("_reward_event_id").alias("_reward_event_water_uL"),
        )
        ...
        .with_columns(
            ...
            .then(0.0)
            .otherwise(pl.col("torque_N_cm"))
            .alias("torque_N_cm")
        )
    )

    # Handle traveled_distance_cm cleanup if the encoder data column exists:
    if "traveled_distance_cm" in behavior_data.columns:
        behavior_data = (
            behavior_data
            ...
            .with_columns(
                (pl.col("system_state") != "idle").cum_sum().alias("_past_idle")
            )
            .with_columns(
                pl.when((pl.col("system_state") == "idle") & (pl.col("_past_idle") == 0))
                .then(0.0)
                .when(pl.col("system_state") == "run")
                .then(pl.col("traveled_distance_cm"))
                .otherwise(None)
                .forward_fill()
                .fill_null(0.0)
                .alias("traveled_distance_cm"),

                # Fixes running speed at the same time
                pl.when(pl.col("system_state") == "run")
                .then(pl.col("running_speed_cm_s"))
                .otherwise(0.0)
                .alias("running_speed_cm_s")
            )
            .drop("_past_idle")
        )
    ...
    return (
        ...
        .with_columns(
            pl.col("torque_N_cm").cast(pl.Float32) if "torque_N_cm" in columns_to_select else None
        )
    )
...
def assemble_session_data(
    ...
    if session_type == SessionTypes.MESOSCOPE_EXPERIMENT:
        ...
        # Assembles the behavior dataset.
        behavior_data = _assemble_behavior_dataset(
            session_data_path=session_data_path, reference_time=fluorescence_data["time_us"].to_numpy()
        )
        # Drops the time column to avoid duplication with fluorescence dataset.
        behavior_data = behavior_data.drop("time_us", "elapsed_minutes")

        # Assembles the experiment dataset.
        experiment_data = _assemble_experiment_dataset(
            session_data_path=session_data_path, reference_time=fluorescence_data["time_us"].to_numpy()
        )
        # Drops the time and traveled distance columns to avoid duplication with fluorescence and behavior datasets.
        experiment_data = experiment_data.drop("time_us", "traveled_distance_cm")

        # Concatenates the dataframes into a unified analysis dataset.
        ...
        with pl.Config(
            ...
            print(behavior_data.slice(offset=2800, length=500))
