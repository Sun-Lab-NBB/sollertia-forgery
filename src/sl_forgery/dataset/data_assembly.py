...
def _assemble_experiment_dataset(session_data_path: Path, reference_time: NDArray[np.uint64]) -> pl.DataFrame:
    ...
    trial_df = trial_df.with_columns(
        pl.int_range(start=1, end=len(trial_df) + 1, dtype=pl.UInt32).alias("trial")
    )
...
def assemble_session_data(
    ...
    if session_type == SessionTypes.MESOSCOPE_EXPERIMENT:
        ...
        # Drops the time columns to avoid duplication with fluorescence dataset.
