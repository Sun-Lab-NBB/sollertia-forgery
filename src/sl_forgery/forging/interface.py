...
def forge_dataset(
    ...
    *,
    ...
    reprocess: bool = False,
    keep_job_logs: bool = False,
    suite2p_configuration_file: str = "GCaMP6f_CA1_MD.yaml",
    processing_batch_size: int = 4,
) -> None:
    """Resolves and executes the necessary dataset forging pipelines for the target project.

    This function acts as the main entry point for all dataset forging in the Sun lab. As part of its runtime, it first
    ...
    Args:
        ...
        process_multiday: Determines whether to execute the multi-day suite2p processing pipeline.
        ...
        suite2p_configuration_file: Specifies the name of the configuration file for the multi-day suite2p processing
...
def _construct_report_pipeline(
    manifest: ProjectManifest,
    project: str,
    session: str,
    server: Server,
    *,
    ...
    """Generates and returns the ProcessingPipeline instance used to execute the report dataset assembly pipeline
    ...
    Returns:
        The configured ProcessingPipeline instance if the session can be processed with this pipeline.
        Otherwise, returns a string describing why the session was excluded from processing.
    """
    # Resolves the path to the local Sun lab working directory.
    local_working_directory = get_working_directory()

    # Validates session eligibility using the standard eligibility checker.
    exclusion_reason = check_session_eligibility(
        manifest=manifest,
        session=session,
        pipeline=ProcessingPipelines.REPORT,
        server=server,
        supported_systems={AcquisitionSystems.MESOSCOPE_VR},
        supported_sessions={SessionTypes.MESOSCOPE_EXPERIMENT, SessionTypes.LICK_TRAINING, SessionTypes.RUN_TRAINING},
        allow_reprocessing=True,  # Report can always be regenerated
    )
    if exclusion_reason is not None:
        return exclusion_reason

    # Resolves the animal for this session.
    animal = manifest.get_animal_for_session(session=session)

    # Resolves the remote session path.
    ...
    job_name = f"{session}_report_assembly"
    job_id = ProcessingTracker.generate_job_id(session_path=remote_session_path, job_name=job_name)

    # Resolves the job working directory on the remote server.
    working_directory = get_remote_job_work_directory(
        server=server, job_name=job_name, pipeline_name=ProcessingPipelines.REPORT
    )
    server.create(remote_path=working_directory, is_dir=True)

    # Creates the SLURM job.
    job = Job(
        job_name=job_name,
        output_log=working_directory.joinpath("output.txt"),
        error_log=working_directory.joinpath("errors.txt"),
    ...
    job.add_command(f"sl-process report -sp {remote_session_path} -id {job_id}")

    # Resolves the paths to the local and remote job tracker files.
    remote_tracker_path = remote_session_path.joinpath("tracking_data", "report_tracker.yaml")
    local_tracker_path = local_working_directory.joinpath(project, f"{session}_report", "report_tracker.yaml")
...
def generate_report_datasets(
    ...
    *,
    ...
    """Generates report datasets for the target sessions on the remote compute server.

    This function constructs and executes ProcessingPipeline instances for each eligible session to generate behavior
    report datasets containing synchronized camera timestamps, behavior data, and experiment data.

    Args:
        manifest_path: The path to the project's manifest .feather file.
        project: The name of the project whose sessions to process.
        sessions: A tuple of SessionMetadata instances defining the sessions to process.
        keep_job_logs: Determines whether to keep completed job logs on the server or (default) remove them after
            each processing pipeline completes successfully. If the pipeline fails, the job logs are kept regardless
            of the value of this argument.
        processing_batch_size: The number of processing pipelines that can be submitted to the remote compute server at
            a time.
    """
    console.echo(message=f"Initializing '{project}' report dataset generation...", level=LogLevel.INFO)
    ...
    # Tracks all pipelines executed for final outcome reporting.
    ...
    for session_metadata in tqdm(sessions, desc="Resolving report pipelines", unit="session"):
        ...
        if isinstance(result, str):
            exclusions[session_metadata.session] = (session_metadata, result)
        else:
            pipelines.append(result)

    # Executes the report generation pipelines.
    successful_count = 0
    failed_count = 0
    if pipelines:
        successful_count, failed_count = execute_pipelines(
    ...
    # Displays the overall processing summary message.
    message = (
        f"Report dataset generation: Complete. Successfully completed {successful_count} pipelines, "
        f"failed {failed_count} pipelines, excluded {total_excluded} sessions. "
        f"The details about the processing outcome are available below:"
    )
    console.echo(message=message, level=LogLevel.INFO)
    ...
    for pipeline in pipelines:
        if pipeline.pipeline_status == ProcessingStatus.FAILED:
            message = f"Report generation for session '{pipeline.session}': Failed."
            console.echo(message=message, level=LogLevel.ERROR)
        elif pipeline.pipeline_status == ProcessingStatus.SUCCEEDED:
            message = f"Report generation for session '{pipeline.session}': Complete."
            console.echo(message=message, level=LogLevel.SUCCESS)

    # Prints exclusion reasons.
    for session_name, (_, reason) in exclusions.items():
        message = f"Report generation for session '{session_name}': Excluded ({reason})."
        console.echo(message=message, level=LogLevel.WARNING)

    console.echo(message="Report generation: Complete.", level=LogLevel.SUCCESS)
