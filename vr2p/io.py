...
def _process_behavior_data(
    ...
    for key, feather_name in feather_files.items():
        ...
        if file_path.exists():
            ...
            zarr_file.require_group(f"behavior/{key}").create_dataset(
                str(counter),
                data=df_array
            )
...
def process_session_data(data_info: dict[str, Any], settings: dict[str, Any], sessions_data) -> None:
    ...
    first_multiday_folder = sessions_data[0].processed_data.mesoscope_data_path / data_info["data"]["suite2p_folder"] / data_info["data"]["multiday_output_folder"]
    ...
    with zarr.open(zarr_folder.as_posix(), mode="w") as f:
        ...
        for counter, sess_data in tqdm(enumerate(sessions_data_ord), desc="Processing session", total=nsessions):
            multiday_folder = sess_data.processed_data.mesoscope_data_path / data_info["data"]["suite2p_folder"] / data_info["data"]["multiday_output_folder"]
            ...
            singleday_path = sess_data.processed_data.mesoscope_data_path / data_info["data"]["suite2p_folder"] / "combined"
            _process_single_session(f, singleday_path, settings, counter)

            # Process multi-session data if available
            if multiday_exists:
                # Load registration data
                backwards_deformed_masks = np.load(multiday_folder / "backwards_deformed_cell_masks.npy", allow_pickle=True)
                ...
                _process_multi_session(
                    f, multiday_folder,
                    backwards_deformed_masks, trans_images, original_images, counter,
                    settings
                )
            else:
                _process_individual_session(
                    f, singleday_path, cell_templates, counter
                )
